...
describe('TaskList', () => {
  ...
  describe('adding tasks - error handling', () => {
  it('shows error when createTask fails and does not add task', async () => {
    const user = userEvent.setup();

    mockedTaskApi.fetchTasks.mockResolvedValue([]);
    mockedTaskApi.createTask.mockRejectedValue(
    ...
    ).toHaveTextContent(/failed to add task/i);

    // Task should NOT appear in the list
    expect(
      screen.queryByText('Broken task')
